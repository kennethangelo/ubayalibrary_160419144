...
class AuthorViewModel (application: Application): AndroidViewModel(application) {
    ...
    val bookAuthorLD = MutableLiveData<List<Book>>()
    //Live data that "emit" error status (boolean)
    val authorLoadErrorLD = MutableLiveData<Boolean>()    //Live data that "emit" data loading status (boolean)
    val bookLoadErrorLD = MutableLiveData<Boolean>()
    //MutableLiveData -> type of updateable LiveData
    val loadingLD = MutableLiveData<Boolean>()
    val loadingBookLD = MutableLiveData<Boolean>()
...
//        val url2 = "http://192.168.0.8/ubayalibrary/book.php?author=$id";
        ...
        val stringRequest2 = StringRequest(
            Request.Method.GET, url2,
            {
                ...
                //Update the student LD which is being observed by Booklist List Fragment
                bookAuthorLD.value = result
                loadingBookLD.value = false
                Log.d("showvolley", result.toString())
            },
            {
                Log.d("showerror", it.toString())
